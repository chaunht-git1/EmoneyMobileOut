...
public class DmquanlyimageDao {
     ...
     public String updatettkhtodate(VwDmQuanlydotimagectietWeb vdqw,String reupdate ) throws SQLException {
        ...
        String fnCall = "{?= call KH.PKS_HOIBAO_WEB.CAPNHAT_DKHTTODAY_2018(?,?,?,?,?,?,?,?)}";
        try {
...
public String updatebiennhanerror(String khoa, String loaict,String soct,String sobn,String reupdate,Date ngaycap,String makhachhang ) throws SQLException {
        ConnectionProvider.reconnectdbastatic();
        Connection con = ConnectionProvider.getCon();
        String kq = null;
        String userid = SessionBean.getUserId();

        String fnCall = "{?= call KH.PKS_HOIBAO_WEB.CAPNHATBIENNHANERROR_2018(?,?,?,?,?,?,?,?,?)}";
        try {
            CallableStatement stm = con.prepareCall(fnCall);
