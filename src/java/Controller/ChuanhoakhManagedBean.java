...
public class ChuanhoakhManagedBean implements Serializable  {
    ...
    public  void  dskhachhangchuachuan(String userid){
     ...
     /*   for (int i = 0; i < listkhang.size(); i++) {
             
           KhachhangchuanhoaModel model= new KhachhangchuanhoaModel();
           model=listkhang.get(i);
           String makh= model.getCustomerCode();
           if (makh.equalsIgnoreCase("NOVALUE"))
           {     
               String kqkh=functionCustomer.kiemtraKhachhang(model);
               int stt=listkhang.indexOf(model);
               model.setCustomerCode(kqkh);
               listkhang.set(stt, model);
    ...
    public String capnhatkhachhang2018(Customer customer,String userid  ) {
          
          Customer customertemp= new Customer();
          customertemp=customer;
          DateFormat dateFormat;
          dateFormat=new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
          Date dateOfBirth=customer.getDateOfBirthDateValue();
          customertemp.setDateOfBirth(dateFormat.format(customer.getDateOfBirthDateValue()) );
          customertemp.setDateOfIssue(dateFormat.format(customer.getDateOfIssueDateValue()) );
